...
현재 파일의 모든 인용 라인은 `nl -ba <file>`로 확인할 수 있다. 이 보고서와 함께
실행한 별도 검증 명령은 표의 파일 경로가 위 Git 출력에 존재하는지, 삭제 표가 빈
결과와 일치하는지, 표에 기록한 `파일:시작–끝` 라인이 현재 파일 범위인지 검사했다.
`scripts/verify_refactor_ledger.py` 자체는 보고서 표가 아니라 stage manifest/ledger와
증거 hash를 검증하는 S01 도구다.
