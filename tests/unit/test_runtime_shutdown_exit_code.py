...
Reviewer 2 audit C-6 / 01§C-1 flagged ``os._exit(0)`` in
``marslab/runtime/stage2_loop.py`` (and the sibling pattern in
``scripts/phase1/run_stage4.py``) because it reports success to the
shell when ``simulation_app.close()`` raises. The fix replaces ``0``
with ``1`` so CI / pytest observe a genuine failure. The ``os._exit``
call itself is retained (not ``sys.exit``) to bypass Kit's ``atexit``
...
Isaac Sim / rclpy imports that are not offline-importable, so we instead
lock in the source-level invariant (no ``os._exit(0)`` in the shutdown
teardown block). Reverting to ``os._exit(0)`` will fail CI.
"""
...
def test_stage2_loop_uses_exit_code_1_on_close_failure():
    ...
    assert (
        "os._exit(0)" not in tail
    ), "stage2_loop.py teardown reverted to os._exit(0) — audit C-6 regressed."


def test_run_stage4_uses_exit_code_1_on_close_failure():
    """``scripts/phase1/run_stage4.py`` must use ``os._exit(1)``."""
    source = _load_source("scripts/phase1/run_stage4.py")
    assert "simulation_app.close()" in source
    assert "os._exit(1)" in source, (
        "run_stage4.py shutdown path must use os._exit(1) — reverting to "
    ...
    assert (
        "os._exit(0)" not in tail
    ), "run_stage4.py teardown reverted to os._exit(0) — see audit C-6."
