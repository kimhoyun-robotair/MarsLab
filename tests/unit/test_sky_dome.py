...
def test_butterscotch_at_low_tau():
    ...
    sky = compute_sky_dome_params(0.3, "assets/mars_sky/")
...
def test_brightness_positive(tau: float) -> None:
    ...
    sky = compute_sky_dome_params(tau, "assets/mars_sky/")
    assert sky.brightness > 0


def test_brightness_decreases_with_tau():
    """Brightness decreases as dust increases."""
    b_low = compute_sky_dome_params(0.1, "assets/mars_sky/").brightness
    b_high = compute_sky_dome_params(3.0, "assets/mars_sky/").brightness
    assert b_low > b_high
...
def test_returns_sky_dome_params():
    sky = compute_sky_dome_params(0.5, "assets/mars_sky/")
    assert isinstance(sky, SkyDomeParams)


def test_negative_tau_raises():
    with pytest.raises(ValueError, match="tau"):
        compute_sky_dome_params(-0.1, "assets/mars_sky/")
