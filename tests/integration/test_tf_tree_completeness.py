...
4. **Each sensor's static TF rotation is identity** — the static
   ...
   need to be updated to cross-check ``local_orientation_rpy_deg``.
...
Sprint task G (Day 2, 2026-04-25) — author writes the test, the user
runs it.
"""
...
# Position tolerance in meters (Reviewer 2 sprint task G acceptance).
POS_TOL_M = 0.01
# Orientation tolerance in degrees (per task G).  Currently the static
# broadcaster emits identity rotation, so any non-trivial assertion is
# trivially within tolerance.
ROT_TOL_DEG = 0.1
# How long to wait for messages on /tf and /tf_static.
TF_BUDGET_S = 30.0
# rclpy spin-once timeout per loop tick.
SPIN_TIMEOUT_S = 0.01

# Frame map: YAML sensor key -> (broadcast child_frame_id, signed-Y/Z flip).
# Mirrors ``scripts/phase1/run_stage4.py:244-250`` and
...
def _flip_yz(xyz: List[float]) -> Tuple[float, float, float]:
    """Apply the body-frame Y/Z flip the static broadcaster does.

    Reproduces ``marslab/ros2_bridge/tf_broadcaster.py:50-52``.
    """
...
def test_tf_tree_contains_all_sensor_frames() -> None:
    ...
    and ``/tf`` (live joint chain), and confirms that every sensor
    ...
    try:
        ...
        # Build the same sensor_frames list run_stage4.py does.  We need
        # this both to drive the static broadcaster and to compare the
        # received TF translation against the YAML.
        ...
        # Capture every TransformStamped sent on /tf and /tf_static.
        ...
        sub_live = bridge.node.create_subscription(TFMessage, "/tf", _on_live, 10)
        ...
        assert not missing, (
            ...
            f"in scripts/phase1/run_stage4.py:244-250."
        )

        # --- Assertion 2: /tf received at least one joint-chain message ---
        assert live_msgs, (
        ...
        for tf in static_msgs:
            if tf.child_frame_id in expected_children:
                assert tf.header.frame_id == "base_link", (
                    ...
                    f"(marslab/ros2_bridge/tf_broadcaster.py:30)."
                )

        # --- Assertion 4: translation matches YAML within 1 cm -------------
        # Build expected map from YAML, applying the documented Y/Z flip.
        expected_xyz: Dict[str, Tuple[float, float, float]] = {}
        for child, xyz in sensor_frames:
            expected_xyz[child] = _flip_yz(xyz)
        ...
        for tf in static_msgs:
            ...
            for axis_idx, axis_name in enumerate(("x", "y", "z")):
                ...
                assert delta <= POS_TOL_M, (
                    ...
                    f"(YAML w/ Y/Z flip), delta {delta:.4f} > {POS_TOL_M} m. "
                    f"Check rover_m2020.yaml local_translation and "
                    f"tf_broadcaster.py:50-52 sign convention."
                )

        # --- Assertion 5: rotation within 0.1 deg of identity --------------
        # The static broadcaster currently authors identity quaternions
        # for sensors (tf_broadcaster.py:53-56).  Once orientation is
        # added, replace this with a YAML cross-check.
        for tf in static_msgs:
